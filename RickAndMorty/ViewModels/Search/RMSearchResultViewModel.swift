...
//  RickAndMorty
//
//  Created by  Ghadi on 12/11/2024.
//
                     ...
                     case .episodes(let existingResults):
                         RMService.shared.execute(request, expecting: RMGetAllEpisodesResponse.self) { [weak self] result in
                             ...
                             case .success(let responseModel):
                                 ...
                                 let additionalResults = moreResults.compactMap({
                                     return RMCharcterEpisodeCollectionViewCellViewModel(episodeDataUrl: URL(string: $0.url))
                                     
                                 })
                                 var newResults: [RMCharcterEpisodeCollectionViewCellViewModel] = []
                                 newResults = existingResults + additionalResults
                               ...
                               enum RMSearchResultType {
                                   ...
                                   case episodes([RMCharcterEpisodeCollectionViewCellViewModel])
